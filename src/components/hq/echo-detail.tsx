...
import { AlertTriangle, Heart, Lightbulb, Loader2, MessageCircleHeart, Play } from "lucide-react";
...
import { ECHO_STATE_LABELS } from "@/lib/hq/mock-data";
...
export function EchoAgentDetail({ agent, onClose }: { agent: HQAgent; onClose: () => void }) {
  ...
  function handleScan() {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        setResult(`VoC scan complete — ${res.feedbackCount} feedback items, ${res.featureRequestCount} feature requests.`);
        router.refresh();
      } else setError(res.error);
    });
  }

  const stateLabel = ECHO_STATE_LABELS[agent.agentState ?? "idle"];
    ...
    <div className="space-y-5">
      ...
      {agent.stats && (
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: "Feedback", key: "totalFeedback", icon: MessageCircleHeart },
            { label: "Positive %", key: "positivePct", icon: Heart },
            { label: "Urgent", key: "urgentCount", icon: AlertTriangle },
            { label: "Top Request", key: "topFeatureFrequency", icon: Lightbulb },
          ].map(({ label, key }) => (
            <div key={key} className="rounded-xl border border-rose-100 bg-white p-3">
              <p className="text-[10px] text-brand-muted">{label}</p>
              <p className="font-heading text-lg font-bold text-brand-primary">
                {key === "positivePct" ? `${agent.stats?.[key] ?? 0}%` : agent.stats?.[key] ?? 0}
              </p>
            </div>
          ))}
        </div>
      ...
      <section>
        ...
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-brand-primary/10">
          <div className="h-full rounded-full bg-rose-800 transition-all" style={{ width: `${agent.progress}%` }} />
        </div>
      </section>

      <p className="rounded-xl border border-rose-100 bg-rose-50/50 px-3 py-2 text-xs text-rose-900">
        Echo never responds to users. Echo provides insights — humans decide what actions to take.
      </p>
      ...
      {result && <p className="text-sm text-rose-800">{result}</p>}
      ...
      <div className="flex gap-2 pt-2">
        ...
        <Button variant="secondary" onClick={() => { onClose(); window.location.href = "/echo"; }}>
          Open Echo
        </Button>
      </div>
    </div>
