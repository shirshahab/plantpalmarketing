...
import { Loader2, Play, Telescope, TrendingUp, Users } from "lucide-react";
...
import { ATLAS_STATE_LABELS } from "@/lib/hq/mock-data";
...
function ActivityLog({ items }: { items: AgentActivityLog[] }) {
  ...
  return (
    <div className="space-y-2">
      {items.map((log) => (
        <div key={log.id} className="rounded-xl border border-sky-100 bg-sky-50/40 px-3 py-2">
...
export function AtlasAgentDetail({ agent, onClose }: { agent: HQAgent; onClose: () => void }) {
  ...
  function handleBrief() {
    ...
    startTransition(async () => {
      ...
      if (res.ok) {
        setResult(`Brief generated — ${res.recommendationsCount} recommendations, ${res.experimentsCount} experiments.`);
        router.refresh();
      } else setError(res.error);
    });
  }

  const stateLabel = ATLAS_STATE_LABELS[agent.agentState ?? "idle"];
    ...
    <div className="space-y-5">
      ...
      {agent.stats && (
        <div className="grid grid-cols-2 gap-2">
          {[
            { label: "Users", key: "totalUsers", icon: Users },
            { label: "30d Forecast", key: "forecast30d", icon: TrendingUp },
            { label: "Recommendations", key: "totalRecommendations", icon: Telescope },
            { label: "Bottlenecks", key: "activeBottlenecks", icon: TrendingUp },
          ].map(({ label, key }) => (
            <div key={key} className="rounded-xl border border-sky-100 bg-white p-3">
              <p className="text-[10px] text-brand-muted">{label}</p>
              <p className="font-heading text-lg font-bold text-brand-primary">
                {typeof agent.stats?.[key] === "number" && key === "totalUsers"
      ...
      <section>
        ...
        <div className="mt-2 h-2 overflow-hidden rounded-full bg-brand-primary/10">
          <div className="h-full rounded-full bg-sky-700 transition-all" style={{ width: `${agent.progress}%` }} />
        </div>
      </section>

      <p className="rounded-xl border border-sky-100 bg-sky-50/50 px-3 py-2 text-xs text-sky-900">
        Atlas identifies growth opportunities and bottlenecks. Atlas never executes — humans decide.
      </p>
      ...
      {result && <p className="text-sm text-sky-800">{result}</p>}
      ...
      <div className="flex gap-2 pt-2">
        ...
        <Button variant="secondary" onClick={() => { onClose(); window.location.href = "/atlas"; }}>
          Open Atlas
        </Button>
      </div>
    </div>
